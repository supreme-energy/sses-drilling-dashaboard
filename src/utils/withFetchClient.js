...
import { getFetchClientFromContext } from "react-powertools/data/FetchClientProvider";
import withData from "react-powertools/data/withData";

const EMPTY = {};

function compareFetchArgs(aa, bb) {
  return (
    aa.path === bb.path &&
    aa.method === bb.method &&
    aa.cache === bb.cache &&
...
function argsEqual(a, b) {
  ...
  if (a.fetchClient !== b.fetchClient) {
    return false;
  }
  ...
  // Handle the case where we have an array of fetchArgs
  const aaIsArray = Array.isArray(aa);
  const bbIsArray = Array.isArray(bb);

  if (aaIsArray !== bbIsArray) {
    return false;
  }

  if (aaIsArray) {
    return aa.length === bb.length && aa.every((aaa, i) => compareFetchArgs(aaa, bb[i]));
  }

  return compareFetchArgs(aa, bb);
}
 ...
 *   Can return an array of query parameters, in which case each entry in the array will be fetched and the result
...
export default function withFetchClient(path, propsToQuery, options = EMPTY) {
  ...
  console.log("WITHFETCHCLIENT", path, propsToQuery, options);
  ...
  function propsToArgs(props, context, prevResult) {
    ...
    const fetchArgs = Array.isArray(query)
      ? query.map(q => constructFetchArgs(q, defaultQuery))
      : constructFetchArgs(query, defaultQuery);
  ...
  function argsToPromise(args, isPolling, prevResult, props, context) {
    ...
    function runSingleFetch(fetchArgs) {
      let fa = fetchArgs;
      if (isPolling) {
    ...
    if (Array.isArray(fetchArgs)) {
      const promises = fetchArgs.map(runSingleFetch);
      promise = Promise.all(promises);
    } else {
      promise = runSingleFetch(fetchArgs);
    }
