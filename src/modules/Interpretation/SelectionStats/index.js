...
import { useComputedSegments, useSelectedWellLog, useComputedDraftSegmentsOnly, useLogExtent } from "../selectors";
...
export default function SelectionStatsContainer() {
  ...
  return (
    <Box display="flex" flexDirection="column" className={css.root}>
      ...
      <Box display="flex" flexDirection="row">
        <SelectionStats selection={[computedSelectedWellLog]} mr={2} gammaRange={selectedWellLogGammaRange} />
        {draftMode ? (
          <SelectionStats selection={computedSegments} gammaRange={pendingSegmentsRange} />
        ) : (
