  ...
  return (
    <div className={classes.percentageBar} ref={containerRef}>
      <div className={classNames("layout horizontal flex", classes.bars)}>
        {values.map(v => {
          console.log("width", scale(v.value), v.value);
          return <div style={{ width: `${scale(v.value)}px`, height: 5, backgroundColor: v.color }} />;
        })}
      </div>
    </div>
  );
        ...
        <React.Fragment>
          ...
          <div className="layout horizontal space-between">
            ...
            <div className="layut vertical">
              ...
              <PercentageBar
                values={[{ value: data.avgSliding, color: "#AFB42B" }, { value: data.avgRotating, color: "#827717" }]}
              />
            </div>
          </div>
          <div className="layout horizontal space-between">
